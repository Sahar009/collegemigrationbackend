...
export const getSingleTuitionPayment = async (paymentId) => {
    try {
        ...
        if (!payment) {
            return messageHandler("Tuition payment not found", false, BAD_REQUEST);
        }

        // Format the response (reuse formatting from getTuitionPayments if you want)
        ...
        return messageHandler("Tuition payment retrieved successfully", true, SUCCESS, formattedPayment);

    } catch (error) {
        console.error('Get single tuition payment error:', error);
        return messageHandler(error.message || "Error retrieving tuition payment", false, BAD_REQUEST);
    }
};